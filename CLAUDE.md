...
**Stack:** React.js, framer motion gsap. GSAP for all animations.
...
- premium, futuristic but calm
- not aggressive, feels clinical and trustworthy
...
<!-- In <head> -->
<link
  href="https://fonts.googleapis.com/css2?family=Sora:wght@200;300;400;600;700;800&family=Playfair+Display:ital,wght@0,400;0,700;1,300;1,400;1,700&display=swap"
  rel="stylesheet"
/>
...
.hero-tag {
  ...
  margin-bottom: 32px;
  opacity: 0;
  transform: translateY(20px); /* GSAP animates this in */
}
.tag-dot {
  width: 6px;
  height: 6px;
...
@keyframes pulse-dot {
  ...
  50% {
    opacity: 0.5;
    box-shadow: 0 0 0 6px rgba(155, 107, 255, 0);
  }
}
```
...
```html
<h1 class="hero-h">
  <span class="hw">Manage</span> <span class="hw">Your</span>
  <span class="hw">Entire</span>
</h1>
...
```css
.fpill {
  position: absolute; /* glassmorphism styling */
}
.fp1 {
  top: -20px;
  left: -20px;
  animation: fp1 3.5s ease-in-out infinite;
}
.fp2 {
  bottom: -16px;
  right: -16px;
  animation: fp2 4s ease-in-out infinite;
}
.fp3 {
  top: 30%;
  right: -30px;
  animation: fp1 5s ease-in-out infinite 0.5s;
}
...
<div id="scroll-text-sec">
  ...
  <div class="scroll-sticky">
    ...
    <div class="big-words" id="bw">Manage <em>Everything</em></div>
...
.float-card {
  ...
  padding: 20px;
  opacity: 0;
  transform: translateY(60px) scale(0.9); /* GSAP animates */
...
.fg-card {
  ...
  opacity: 0;
  transform: translateY(28px); /* GSAP animates */
}
.fg-card:hover {
...
.dp-f {
  background: rgba(100, 200, 140, 0.7);
} /* full */
.dp-p {
...
- **Headline:** "Your Meta ads, _finally connected._"
...
_End of CLAUDE.md — This file contains the complete design system for GloryWellNic landing page v1.0_
